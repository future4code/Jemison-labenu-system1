...
import { StudentsDatabase } from './../database/studentDatabase';
...
export const CreateStudents = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const emailExists = await studentDatabase.StudentEmailExist(email)
        ...
        for (let hobby of hobbies) {
            const hobbyExists = await hobbyDatabase.HobbyExists(hobby)
            if (hobbyExists.length == 0) {
                let newHobby = new HobbyClass(
                    0,
                    hobby
                )
                hobbyDatabase.CreateHobby(newHobby)
            }
        }

        for (let hobby of hobbies) {
            let hobbyExists = await hobbyDatabase.HobbyExists(hobby)
